...
public:
	...
	void store_data(player_t* e, bool store = true)
	{
		...
		if (store)
		{
			...
			memcpy(matrixes_data.main, player->m_CachedBoneData().memory, player->m_CachedBoneData().allocationCount * sizeof(matrix3x4_t));
		}
		...
		bone_count = player->m_CachedBoneData().allocationCount;
	...
	void adjust_player()
	{
		...
		memcpy(player->m_CachedBoneData().memory, matrixes_data.main, player->m_CachedBoneData().allocationCount * sizeof(matrix3x4_t)); //-V807

		player->m_fFlags() = flags;
		player->m_CachedBoneData().size = bone_count;
