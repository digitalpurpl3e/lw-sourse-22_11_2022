...
class fxClientClass/*no comments*/ {
public:
...
void weather()
{
	...
	g_ctx.globals.m_networkable = reinterpret_cast<fxClientClass*>(client_class)->m_pCreateFn(entry, serial);
...
void __stdcall hooks::hooked_fsn(ClientFrameStage_t stage)
{
	...
	int f = stage;
	InventoryChanger::run((FrameStage)f);
	...
	if (stage == FRAME_RENDER_START)
	{
		if (g_cfg.esp.client_bullet_impacts)
		{
			...
			auto& client_impact_list = *(zUtlVector <client_hit_verify_t>*)((uintptr_t)g_ctx.local() + 0x11C50);

			for (auto i = client_impact_list.allocationCount; i > last_count; --i)
			...
			if (client_impact_list.allocationCount != last_count)
				last_count = client_impact_list.allocationCount;
		}
	...
	if (stage == FRAME_NET_UPDATE_END)
	{
		...
		auto v1 = g_ctx.shots.begin();
		for (auto& shot = v1; shot != g_ctx.shots.end(); ++shot)
		{
			if (shot->end)
			{
				current_shot = shot;
