	...
	else if (!strcmp(event_name, crypt_str("bullet_impact")))
	{
		...
		if (user == m_engine()->GetLocalPlayer())
		{
			...
			if (current_shot && aim::get().last_target[current_shot->last_target].record.player->valid(true, false)) //-V807
			{
				...
				{
					memcpy(aim::get().last_target[current_shot->last_target].record.player->m_CachedBoneData().memory, aim::get().last_target[current_shot->last_target].record.matrixes_data.zero, aim::get().last_target[current_shot->last_target].record.player->m_CachedBoneData().allocationCount * sizeof(matrix3x4_t)); //-V807
					...
					memcpy(aim::get().last_target[current_shot->last_target].record.player->m_CachedBoneData().memory, aim::get().last_target[current_shot->last_target].record.matrixes_data.first, aim::get().last_target[current_shot->last_target].record.player->m_CachedBoneData().allocationCount * sizeof(matrix3x4_t)); //-V807
					...
					memcpy(aim::get().last_target[current_shot->last_target].record.player->m_CachedBoneData().memory, aim::get().last_target[current_shot->last_target].record.matrixes_data.second, aim::get().last_target[current_shot->last_target].record.player->m_CachedBoneData().allocationCount * sizeof(matrix3x4_t)); //-V807
	...
	else if (!strcmp(event_name, crypt_str("player_hurt")))
	{
		...
		if (attacker_id == m_engine()->GetLocalPlayer() && user_id != m_engine()->GetLocalPlayer())
		{
			...
			if (weapon_is_aim(weapon))
			{
				...
				otheresp::get().hitmarker.point = entity->hitbox_position_matrix(get_hitbox_by_hitgroup(hitgroup), current_shot && entity == aim::get().last_target[current_shot->last_target].record.player ? aim::get().last_target[current_shot->last_target].record.matrixes_data.main : entity->m_CachedBoneData().memory);
				otheresp::get().damage_marker[user_id] = otheresp::Damage_marker
				{
					entity->hitbox_position_matrix(get_hitbox_by_hitgroup(hitgroup), current_shot && entity == aim::get().last_target[current_shot->last_target].record.player ? aim::get().last_target[current_shot->last_target].record.matrixes_data.main : entity->m_CachedBoneData().memory),
					m_globals()->m_curtime,
					Color::White,
					damage,
					hitgroup
